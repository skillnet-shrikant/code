-- *************************************************************************************
--  Procedure: p_get_inventory_for_order
-- *************************************************************************************
create or replace
procedure p_get_inventory_for_order  (pOrderId in varchar2) 
    as
...
-- ******************************************************************
--                       get_inventory_for_order
--
--   Get the store inventory for all skus in the order
--   Check if there is any inventory across all the stores
-- ******************************************************************	
procedure get_inventory_for_order (pOrderId varchar2) as
	
begin

	-- Get the inventory record for the UPC
	printDebugOutput ('Begin get_inventory_for_order ' || pOrderId); 	
...
STORE_ID                    VARCHAR2(6)  
AVAILABLE_QTY               NUMBER(5)    
REQUESTED_QTY               NUMBER(5)    
CURRENT_QUEUE_SIZE          NUMBER(5)    
...
*/
/*
INVENTORY_ID     NOT NULL VARCHAR2(40) 
STORE_ID         NOT NULL VARCHAR2(40) 
CATALOG_REF_ID   NOT NULL VARCHAR2(40) 
...
select * from atg_core.mff_store_inventory
where catalog_ref_id in (select catalog_ref_id from tmp_order_item);
...
begin
	dbms_output.enable;
	dbms_output.put_line('p_get_inventory_for_order ... ' || pOrderId);
