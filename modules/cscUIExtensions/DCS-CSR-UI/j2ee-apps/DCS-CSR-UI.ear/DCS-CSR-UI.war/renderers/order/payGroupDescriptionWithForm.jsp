...
<c:catch var="exception">
	<dsp:page xml="true">
		...
		<dsp:layeredBundle basename="atg.commerce.csr.order.WebAppResources">
			...
			<dsp:setvalue bean="MFFCSCTasksOnOrderFormHandler.inputSizeRGC"
			...
			<dsp:form method="post" name="${gcform1}" id="${gcform1}"
				...
				<dsp:input type="hidden" priority="-10" value=""
					bean="MFFCSCTasksOnOrderFormHandler.fulfillRefundGiftCard" />
				<dsp:input type="hidden" priority="10"
					value="${fn:length(ciPendingFulfillment)}"
					bean="MFFCSCTasksOnOrderFormHandler.inputSizeRGC" />
					...
					<tbody>
						...
						<c:forEach items="${ciPendingFulfillment}" var="commerceItem"
							varStatus="commerceItemIndex">
								<dsp:include
									src="/panels/order/finish/finishOrderCartPayGroupWithForm.jsp"
			...
			<script type="text/javascript">
				function displayShipLabel(orderId) {
		  	 		popupUrl='${CSRConfigurator.contextRoot}' + '/include/order/displayPackingSlipRefundGC.jsp?_windowid=${windowId}&currentOrder=' + orderId;
