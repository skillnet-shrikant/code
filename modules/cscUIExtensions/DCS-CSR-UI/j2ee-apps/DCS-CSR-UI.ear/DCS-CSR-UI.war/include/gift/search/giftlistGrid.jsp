...
@version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/include/gift/search/giftlistGrid.jsp#1 $$Change: 946917 $
...
<dsp:getvalueof var="triggerInitialLoad" param="triggerInitialLoad" />
<dsp:layeredBundle basename="atg.commerce.csr.Messages">
<dsp:importbean bean="/atg/core/i18n/LocaleTools"/>
<dsp:getvalueof bean="LocaleTools.userFormattingLocaleHelper.datePatterns.short" var="dateDefault"/>
<script type="text/javascript">
dojo.provide("atg.commerce.csr.gift");
atg.commerce.csr.gift.refreshSearchResults = function(isNewSearch) {
  try {
  if(!atg.commerce.csr.order.gift.isSearchFormEmpty("${searchFormId}","${dateDefault}")) {
    var _inst = ${gridInstanceId};
    if (!_inst && !atg.service.form.isFormEmpty("${searchFormId}")) {
      console.debug("no instance, setting up a new one");
      ${gridInstanceId} = atg.commerce.csr.gift.setupResultsGrid();
...
atg.commerce.csr.gift.setupResultsGrid = function () {
  var _inst = ${gridInstanceId};
  if (_inst) { console.debug("grid already set up"); return _inst;}

  var viewLink = "<a href=\"#\" class=\"blueU\" onclick=\"giftlistView('giftlistViewForm','');return false;\">theGiftlistId</a>";
  ...
  params.messages = { inProgress:"<fmt:message key='giftlists.search-in-progress'/>",
                      noResultsFound:"<fmt:message key='giftlists.no-matching-giftlists'/>",
                      resultsFound:"<fmt:message key='giftlists.matching-giftlists'/>"};
...
_container_.onLoadDeferred.addCallback(function () {
  ...
  <c:choose>
  <c:when test="${triggerInitialLoad == true}">
    atg.commerce.csr.gift.refreshSearchResults(true);
  </c:when>
  <c:otherwise>
    if (!atg.commerce.csr.order.gift.isSearchFormEmpty("${searchFormId}","${dateDefault}")) {
    	atg.commerce.csr.gift.refreshSearchResults(false);
    }
  </c:otherwise>
  </c:choose>
