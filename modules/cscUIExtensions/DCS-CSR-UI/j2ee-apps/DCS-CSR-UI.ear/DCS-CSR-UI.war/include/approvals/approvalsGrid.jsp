<%--
 @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/include/approvals/approvalsGrid.jsp#1 $
...
atg.commerce.csr.approvals.refreshSearchResults = function() {
    try {
    console.debug("Refresh Search Results for ${gridInstanceId}");

    var _inst = atg.commerce.csr.approvals.setupResultsGrid();
    if (!_inst) {
      console.debug("no instance, setting up a new one");
      ${gridInstanceId} = atg.commerce.csr.approvals.setupResultsGrid();
  ...
  atg.commerce.csr.approvals.setupResultsGrid = function () {
      ...
      params.messages = { inProgress:"<fmt:message key='search-in-progress'/>",
                          noResultsFound:"<fmt:message key='no-order-approvals'/>",
                          resultsFound:"<fmt:message key='order-approvals'/>"};
    ...
    _container_.onLoadDeferred.addCallback(function () {
      ...
      atg.commerce.csr.approvals.refreshSearchResults();
    });
...
<%-- @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/include/approvals/approvalsGrid.jsp#1 $$Change: 946917 $--%>
