...
<dsp:page xml="true">
  ...
  <c:if test="${propertyName == 'value1'}">
    ...
    <c:if test="${displayValue == true}">
      <dsp:droplet name="/atg/commerce/custsvc/returns/IsBalancingPaymentGroup">
      <dsp:param name="paymentGroupId" value="${paymentGroup.id}"/>
      <dsp:oparam name="true">
        <fmt:message key="newOrderBilling.displayPaymentMethods.returnCredit"/>
      </dsp:oparam>
  ...
  <c:if test="${propertyName == 'value2'}">
    ...
    <c:if test="${displayValue == true}">
      ...
      <csr:formatNumber value="${remainingAmount}" type="currency" currencyCode="${order.priceInfo.currencyCode}"/>
    </c:if>
  </c:if>
...
<%-- @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/include/order/displayStoreCredit.jsp#2 $$Change: 1179550 $--%>
