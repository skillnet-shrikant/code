<%--
 ...
 @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/panels/order/orderGrid.jsp#1 $
...
atg.commerce.csr.order.refreshSearchResults${gridWidgetId} = function() {
  try {
  ...
  if (!_inst) {
    ...
    ${gridInstanceId} = atg.commerce.csr.order.setupResultsGrid${gridWidgetId}();
...
atg.commerce.csr.order.setupResultsGrid${gridWidgetId} = function () {
  var _inst = ${gridInstanceId};
  if (_inst) { console.debug("grid already set up"); return _inst;}
  var imageClosed = "icon_find.gif";
  var imageOpen = "icon_find.gif";
  var imagePath = "${CSRConfigurator.contextRoot}/images/icons/";
  var detailFormId = "orderDetailForm";

  <c:set var="itemDetailPage" value="${gridConfig.itemDetailPage}"/>
  var detailSuccessURL = "<c:url value="${fn:trim(itemDetailPage.URL)}" context="${fn:trim(itemDetailPage.servletContext)}"
    ><c:param name="_windowid">${windowId}</c:param
    ></c:url>";
  var selectLink = '<a href=\"#\" class=\"blueU\" title=\"<fmt:message key="select-order"/>\" onclick=\"atg.commerce.csr.order.loadExistingOrder(\'theOrderId\',\'orderState\');return false;\"><fmt:message key="table-options-workon"/></a>';
  var viewLink = '<a href=\"#\" class=\"blueU\" title=\"<fmt:message key="view-order"/>\" onclick=\"atg.commerce.csr.order.viewExistingOrder(\'theOrderId\',\'orderState\');return false;\">theOrderId</a>';

  if (dojo.trim("${detailFormId}")) { detailFormId = "${detailFormId}"; }
  ...
  params.messages = { inProgress:"<fmt:message key='search-in-progress'/>",
                      noResultsFound:"<fmt:message key='no-matching-orders'/>",
                      resultsFound:"<fmt:message key='matching-orders'/>"};
  ...
  var popup = [
             { name: " ",
               ...
               get: dojo.hitch(_inst, "getCellDataByPost", "detail", detailFormId,
                 {orderId: dojo.hitch(_inst, "getCellData", "id")}, detailSuccessURL)
             }
  ];
  var view = { cells: [ columns, popup ] };
  var layout = [ view ];
...
<div style="width:100%">
  <div id="${gridWidgetId}"
       ...
       loadContent="atg.commerce.csr.order.refreshSearchResults${gridWidgetId}()">
  </div>
</div>
...
<%-- @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/panels/order/orderGrid.jsp#1 $$Change: 946917 $--%>
