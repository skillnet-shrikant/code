...
<dsp:page xml="true">
  ...
  <dsp:getvalueof var="refundMethodIndex" param="refundMethodIndex" />
		...
		<c:if
			test="${!empty pgTypeConfig && !empty pgTypeConfig.editAppeasementMethodPageFragment}">
			
      <dsp:layeredBundle basename="${pgConfig.resourceBundle}">
       <fmt:message var="editPageFragmentTitle" key="${pgConfig.editAppeasementMethodPageFragmentTitleKey}"/>
      </dsp:layeredBundle>
      
			
			<c:url var="editPaymentOptionURL"
				context="/${pgTypeConfig.editAppeasementMethodPageFragment.servletContext}"
				value="${pgTypeConfig.editAppeasementMethodPageFragment.URL}">
...
<%-- @version $Id: //application/DCS-CSR-UI/version/11.2/src/web-apps/DCS-CSR-UI/panels/order/appeasements/displayAppeasementPmtEndFrag.jsp#1 $$Change: 1179550 $--%>
