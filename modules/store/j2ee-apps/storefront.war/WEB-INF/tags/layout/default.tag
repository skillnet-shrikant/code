...
<c:set var="productId" value="${productId}" scope="request"/>
...
<!--[if (gt IE 9)|!(IE)]><!--><html class="no-js ${deviceClass}" lang="en"><!--<![endif]-->
	<head>
		...
		<c:if test="${!isAjax}">
			...
			<%-- Vimeo: used for videos across the site --%>
	...
	<body class="${bodyClass}" data-controller="${jsController}" data-action="${jsAction}">
				...
				<div class="inner-wrap">
					...
					<c:choose>
						...
						<c:otherwise>
							...
							<c:import url="/sitewide/header.jsp"/>
							<c:import url="/sitewide/headerMobile.jsp"/>
						</c:otherwise>
					</c:choose>

					<aside id="side-cart" class="side-cart">
						<%--<c:import url="/sitewide/sideCart.jsp"/>--%>
						<jsp:include page="/sitewide/sideCart.jsp" />
					</aside>
		...
		<%-- recaptcha --%>
		<c:if test="${(section == 'home' && pageType == 'giftCardBalance') || (section == 'account' && pageType == 'passwordReset')}">
			<%@ include file="/sitewide/third_party/recaptcha.jspf" %>
		</c:if>
