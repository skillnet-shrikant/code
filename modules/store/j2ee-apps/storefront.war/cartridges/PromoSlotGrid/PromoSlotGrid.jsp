				...
				<c:if test="${not empty contentItem.PromoSlots}">
					<div class="section-row ${mobileHideClass} ${desktopHideClass}" ${customContentStyle}>
						<div class="section-content">
							<c:choose>
								<c:when test="${contentItem.promoSlotsPerRow == 1}">
									<dsp:getvalueof var="gridClass" value="promo-grid promo-static-stack-grid-one"/>
								</c:when>
								<c:when test="${contentItem.promoSlotsPerRow == 2}">
									<dsp:getvalueof var="gridClass" value="promo-grid promo-static-stack-grid-two"/>
								</c:when>
								<c:when test="${contentItem.promoSlotsPerRow == 3}">
									<dsp:getvalueof var="gridClass" value="promo-grid promo-static-stack-grid-three"/>
								</c:when>
								<c:when test="${contentItem.promoSlotsPerRow == 4}">
									<dsp:getvalueof var="gridClass" value="promo-grid promo-static-stack-grid-four"/>
								</c:when>
								<c:when test="${contentItem.promoSlotsPerRow == 5}">
									<dsp:getvalueof var="gridClass" value="promo-grid promo-static-stack-grid-five"/>
								</c:when>
								<c:otherwise>
